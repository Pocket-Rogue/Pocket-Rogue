...
rhit.GamePageController = class {
	...
	updateView() {
		...
		const stars = Math.round(2 * rhit.fbSingleGameManager.stars / rhit.fbSingleGameManager.numRatings) / 2;
...
rhit.FbSingleGameManager = class {
	constructor(gameId, userId) {
		...
		this._userGameRef = firebase.firestore().collection(rhit.FB_COLLECTION_USERS).doc(rhit.fbAuthManager.uid).collection("playedGames").doc(gameId);
